...
class Result:
    ...
    avoid_test_duration_ms: int | None = None
...
def parse_at_obstacle(response: str) -> tuple[bool, bool, bool, int, int]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        ...
        return enabled, valid, blocked, distance_mm, threshold_mm
    raise TimeoutError(f"robot obstacle result not found in response: {response!r}")


def parse_at_avoid(response: str) -> tuple[bool, int, Status, bool, bool, bool, int, int]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        ...
        return active, phase, status, enabled, valid, blocked, distance_mm, threshold_mm
    raise TimeoutError(f"robot avoid result not found in response: {response!r}")


def parse_at_avoid_test(response: str) -> tuple[bool, int, Status, bool, bool, bool, int, int, int]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if not line.startswith("+ROBOT:AVOIDTEST,"):
            continue
        fields = line.split(",")
        if len(fields) != 10:
            raise ValueError(f"bad robot avoid test line: {line!r}")
        try:
            active = int(fields[1]) != 0
            phase = int(fields[2])
            status = Status(int(fields[3]))
            ...
            distance_mm = int(fields[7])
            threshold_mm = int(fields[8])
            duration_ms = int(fields[9])
        except ValueError as exc:
            raise ValueError(f"bad robot avoid test values: {line!r}") from exc
        return active, phase, status, enabled, valid, blocked, distance_mm, threshold_mm, duration_ms
    raise TimeoutError(f"robot avoid test result not found in response: {response!r}")
    ...
    def send(self, key: str, speed: int = 45) -> Result:
        ...
        while time.monotonic() < deadline:
            ...
            if raw:
                ...
                if (
                    (
                        "+ROBOT:ACK," in response
                        or "+ROBOT:MOTOR," in response
                        or "+ROBOT:OLED," in response
                        or "+ROBOT:ENV," in response
                        or "+ROBOT:OBS," in response
                        or "+ROBOT:AVOID," in response
                        or "+ROBOT:AVOIDTEST," in response
                        or "+ROBOT:STATE," in response
                    )
        ...
        if normalized == "I":
            ...
            (
                ...
                distance_mm,
                obstacle_threshold_mm,
                avoid_test_duration_ms,
            ) = parse_at_avoid_test(response)
            sequence = 0
            moving = avoid_active
        elif normalized == "T":
        ...
        for result in results:
            writer.writerow(
                (
                    ...
                    "" if result.avoid_test_duration_ms is None else result.avoid_test_duration_ms,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "A=auto obstacle avoidance, X=simulated avoidance test, T=status"
        ),
    )
    ...
    with client_factory() as client:
        for key in keys:
            ...
            print(
                ...
                f"obs={int(result.obstacle_valid)} block={int(result.obstacle_blocked)} "
                ...
                + (
                    ...
                    if result.avoid_phase is None
                        ...
                        + (
                            ""
                            if result.avoid_test_duration_ms is None
                            else f" test_ms={result.avoid_test_duration_ms}"
                        )
                    )
                )
                + (
