...
Copy-Item -Path (Join-Path $overlayRoot 'application\*') -Destination (Join-Path $resolvedSdk 'application') -Recurse -Force
Copy-Item -Path (Join-Path $overlayRoot 'build\*') -Destination (Join-Path $resolvedSdk 'build') -Recurse -Force

if (Test-Path -LiteralPath (Join-Path $overlayRoot 'tools')) {
    Copy-Item -Path (Join-Path $overlayRoot 'tools\*') -Destination (Join-Path $resolvedSdk 'tools') -Recurse -Force
}

if (Test-Path -LiteralPath (Join-Path $overlayRoot 'drivers')) {
    Copy-Item -Path (Join-Path $overlayRoot 'drivers\*') -Destination (Join-Path $resolvedSdk 'drivers') -Recurse -Force
}
