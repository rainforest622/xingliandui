...
def _parse_log_fields(line: str) -> dict[str, int]:
    values: dict[str, int] = {}
...
class Result:
    ...
    patrol_phase: int | None = None
    patrol_leg_index: int | None = None
    patrol_loop_count: int | None = None
    ...
    env_age_ms: int | None = None
    obstacle_age_ms: int | None = None
...
class RobotClient:
    ...
    def send(self, key: str, speed: int = 100) -> Result:
...
def parse_at_ack(response: str) -> tuple[int, Status, bool]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT AT ACK "):
            fields = _parse_log_fields(line)
            return (
                fields.get("seq", 0),
                Status(fields.get("status", Status.OK.value)),
                fields.get("moving", 0) != 0,
            )
...
def parse_at_motor(response: str) -> bool:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT MOTOR "):
            return _parse_log_fields(line).get("ready", 0) != 0
...
def parse_at_env(response: str) -> tuple[bool, int, int]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT ENV "):
            fields = _parse_log_fields(line)
            return (
                fields.get("ok", 0) != 0,
                fields.get("temp_deci", 0),
                fields.get("hum_deci", 0),
            )
...
def parse_at_obstacle(response: str) -> tuple[bool, bool, bool, int, int, int | None]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT OBS "):
            fields = _parse_log_fields(line)
            return (
                fields.get("enabled", 0) != 0,
                fields.get("valid", 0) != 0,
                fields.get("blocked", 0) != 0,
                fields.get("distance", 0),
                fields.get("threshold", 0),
                fields.get("reason"),
            )
...
def parse_at_avoid(response: str) -> tuple[bool, int, Status, bool, bool, bool, int, int, int | None]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT AVOID "):
            fields = _parse_log_fields(line)
            reason = fields.get("reason")
            valid = reason == 0 if reason is not None else False
            return (
                fields.get("active", 0) != 0,
                fields.get("phase", 0),
                Status(fields.get("status", Status.OK.value)),
                True,
                valid,
...
def parse_at_patrol(response: str) -> tuple[bool, int, Status, int, int, int, bool, bool, bool, int, int, int]:
    normalized_response = response.replace("ROBOT \r\nPATROL", "ROBOT PATROL")
    normalized_response = normalized_response.replace("ROBOT \nPATROL", "ROBOT PATROL")
    for field in ("active", "phase", "status", "distance", "threshold", "reason", "alarm", "loop"):
        for split_at in range(1, len(field)):
    ...
    for raw_line in normalized_response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT PATROL ") and "active=" in line:
            fields = _parse_log_fields(line)
            reason = fields.get("reason", 0)
            distance_mm = fields.get("distance", 0)
            threshold_mm = fields.get("threshold", 0)
            valid = reason in (0, 6)
            blocked = valid and threshold_mm > 0 and distance_mm < threshold_mm
            return (
                fields.get("active", 0) != 0,
                fields.get("phase", 0),
                Status(fields.get("status", Status.OK.value)),
        ...
        if not line.startswith("+ROBOT:PATROL,"):
            continue
        fields = line.split(",")
        if len(fields) != 13:
            raise ValueError(f"bad robot patrol line: {line!r}")
        try:
            active = int(fields[1]) != 0
            phase = int(fields[2])
            status = Status(int(fields[3]))
            leg_index = int(fields[4])
            loop_count = int(fields[5])
            alarm_flags = int(fields[6])
            enabled = int(fields[7]) != 0
            valid = int(fields[8]) != 0
            blocked = int(fields[9]) != 0
            distance_mm = int(fields[10])
            threshold_mm = int(fields[11])
            reason = int(fields[12])
        except ValueError as exc:
            raise ValueError(f"bad robot patrol values: {line!r}") from exc
        return (
            active,
            phase,
            status,
    ...
    raise TimeoutError(f"robot patrol result not found in response: {response!r}")
...
def parse_at_state(response: str) -> tuple[int, bool, bool, int, int, int | None]:
    for raw_line in response.splitlines():
        ...
        if line.startswith("ROBOT STATE "):
            fields = _parse_log_fields(line)
            age_ms = fields.get("age", 0xFFFFFFFF)
            if age_ms == 0xFFFFFFFF:
                age_ms = None
            return (
                fields.get("uptime", 0),
                fields.get("ready", 0) != 0,
                fields.get("moving", 0) != 0,
                fields.get("last_cmd", 0),
                fields.get("seq", 0),
                age_ms,
            )
...
def parse_at_monitor(
    response: str,
) -> tuple[int, bool, bool, bool, int, int, bool, bool, bool, int, int, int, int, int, int | None, int | None]:
    for raw_line in response.splitlines():
        line = raw_line.strip()
        if line.startswith("ROBOT MON "):
            fields = _parse_log_fields(line)
            env_age_ms = fields.get("env_age", 0xFFFFFFFF)
            obstacle_age_ms = fields.get("obs_age", 0xFFFFFFFF)
            if env_age_ms == 0xFFFFFFFF:
                env_age_ms = None
            if obstacle_age_ms == 0xFFFFFFFF:
                obstacle_age_ms = None
            return (
                fields.get("uptime", 0),
                fields.get("ready", 0) != 0,
                fields.get("moving", 0) != 0,
        ...
        if not line.startswith("+ROBOT:MON,"):
            continue
        fields = line.split(",")
        if len(fields) != 17:
            raise ValueError(f"bad robot monitor line: {line!r}")
        try:
            uptime_ms = int(fields[1])
            ready = int(fields[2]) != 0
            moving = int(fields[3]) != 0
            env_ok = int(fields[4]) != 0
            temperature_deci_c = int(fields[5])
            humidity_deci_percent = int(fields[6])
            obstacle_enabled = int(fields[7]) != 0
            obstacle_valid = int(fields[8]) != 0
            obstacle_blocked = int(fields[9]) != 0
            distance_mm = int(fields[10])
            threshold_mm = int(fields[11])
            reason = int(fields[12])
            alarm_flags = int(fields[13])
            sample_count = int(fields[14])
            env_age_ms = int(fields[15])
            obstacle_age_ms = int(fields[16])
        except ValueError as exc:
            raise ValueError(f"bad robot monitor values: {line!r}") from exc
        if env_age_ms == 0xFFFFFFFF:
            env_age_ms = None
        if obstacle_age_ms == 0xFFFFFFFF:
            obstacle_age_ms = None
    ...
    raise TimeoutError(f"robot monitor result not found in response: {response!r}")
...
class SerialAtRobotClient:
    ...
    def send(self, key: str, speed: int = 100) -> Result:
        ...
        patrol_active = False
        patrol_phase = None
        patrol_leg_index = None
        patrol_loop_count = None
        uptime_ms = None
        last_command = None
        last_sequence = None
        age_ms = None
        ...
        return Result(
            ...
            patrol_loop_count=patrol_loop_count,
            uptime_ms=uptime_ms,
            ...
            obstacle_age_ms=obstacle_age_ms,
        )
        ...
        for result in results:
            writer.writerow(
                (
                    ...
                    int(result.patrol_active),
                    "" if result.patrol_phase is None else result.patrol_phase,
                    "" if result.patrol_leg_index is None else result.patrol_leg_index,
                    "" if result.patrol_loop_count is None else result.patrol_loop_count,
                    ...
                    "" if result.alarm_flags is None else result.alarm_flags,
                    "" if result.sample_count is None else result.sample_count,
                    "" if result.env_age_ms is None else result.env_age_ms,
                    "" if result.obstacle_age_ms is None else result.obstacle_age_ms,
                )
            )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "A=auto obstacle avoidance, P=route patrol, T=status, M=inspection monitor"
        ),
    )
    ...
    try:
        with client_factory() as client:
            for key in keys:
                ...
                print(
                    ...
                    + (
                        ""
                        if result.patrol_phase is None
                        else (
                            f" patrol={int(result.patrol_active)}"
                            f" phase={result.patrol_phase}"
                            f" leg={result.patrol_leg_index}"
                            f" loop={result.patrol_loop_count}"
                        )
                    )
                    + (
                        ""
                    ...
                    + (
                        ""
                        if result.alarm_flags is None
                        else (
                            f" alarm=0x{result.alarm_flags:x}"
                            f" samples={result.sample_count}"
                            f" env_age={'none' if result.env_age_ms is None else str(result.env_age_ms) + 'ms'}"
                            f" obs_age={'none' if result.obstacle_age_ms is None else str(result.obstacle_age_ms) + 'ms'}"
                        )
                    )
                )
        ...
        if args.transport == "serial-at":
            print(
                ...
                "请确认烧录的是 ws63-liteos-app_wave_rover_bridge_load_only.fwpkg，"
