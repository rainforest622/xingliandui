...
#define ROBOT_OBSTACLE_IDLE_TIMEOUT_US 45000U
...
static bool set_obstacle_out_of_range(robot_obstacle_data_t *output)
{
    output->valid = true;
    output->blocked = false;
    output->distance_mm = ROBOT_OBSTACLE_MAX_DISTANCE_MM;
    output->threshold_mm = ROBOT_OBSTACLE_THRESHOLD_MM;
    output->reason = ROBOT_OBSTACLE_REASON_OUT_OF_RANGE;
    return true;
}
...
bool robot_obstacle_read(robot_obstacle_data_t *output)
{
    ...
    if (uapi_gpio_get_val(ROBOT_OBSTACLE_ECHO_PIN) == GPIO_LEVEL_HIGH) {
        if (!wait_for_echo(GPIO_LEVEL_LOW, ROBOT_OBSTACLE_IDLE_TIMEOUT_US, &echo_end_us)) {
            return set_obstacle_out_of_range(output);
        }
    }
