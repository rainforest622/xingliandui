...
#define ROBOT_SLE_START_DELAY_MS 3000U
...
static int robot_sle_format_monitor_response(char *buffer, size_t size, const robot_mvp_monitor_t *monitor)
{
...
static int robot_sle_format_patrol_response(char *buffer, size_t size, const robot_mvp_patrol_result_t *result)
{
...
static bool robot_sle_parse_uint_field(char **cursor, uint32_t *value)
{
    char *start = NULL;
    char *end = NULL;
...
static bool robot_sle_skip_separator(char **cursor)
{
...
static void robot_sle_execute_route_text(const uint8_t *value, uint16_t length, char *response, size_t response_size)
{
    ...
    if ((value == NULL) || (response == NULL) || (response_size == 0U)) {
        return;
    }
    if (length >= sizeof(text)) {
        (void)snprintf(response, response_size, "+ROBOT:ROUTE,ERR,LEN\r\n");
        return;
    }

    (void)memset_s(text, sizeof(text), 0, sizeof(text));
    if (memcpy_s(text, sizeof(text) - 1U, value, length) != EOK) {
        (void)snprintf(response, response_size, "+ROBOT:ROUTE,ERR,COPY\r\n");
        return;
    }
    text[length] = '\0';
    if (text[0] == 'r') {
    ...
    if ((text[0] != 'R') || (text[1] != ',')) {
        (void)snprintf(response, response_size, "+ROBOT:ROUTE,ERR,FORMAT\r\n");
        return;
    }

    cursor = &text[2];
    if (!robot_sle_parse_uint_field(&cursor, &forward_ms) ||
    ...
    config.forward_ms = forward_ms;
    config.turn_ms = turn_ms;
    config.speed = (uint8_t)speed;
    config.max_loops = loops;
    if (!robot_mvp_control_config_patrol(&config)) {
        (void)snprintf(response, response_size, "+ROBOT:ROUTE,ERR,RANGE\r\n");
        return;
    }
...
static void robot_sle_execute_text(const uint8_t *value, uint16_t length, char *response, size_t response_size)
{
    ...
    if ((value == NULL) || (length == 0U)) {
        (void)snprintf(response, response_size, "+ROBOT:ERR,EMPTY\r\n");
        return;
    }
    ...
    robot_sle_execute_key(key, response, response_size);
}
...
static void robot_sle_execute_key(uint8_t key, char *response, size_t response_size)
{
    ...
    switch (key) {
        ...
        case 'D': {
            ...
            (void)snprintf(response, response_size, "+ROBOT:OBS,%u,%u,%u,%u,%u,%u\r\n",
        ...
        case 'M': {
            ...
            (void)robot_sle_format_monitor_response(response, response_size, &monitor);
            return;
        }
        case 'A': {
    ...
    errcode_t status)
{
    ...
    robot_sle_send_write_response(conn_id, write_cb, ERRCODE_SLE_SUCCESS);
    (void)memset_s(response, sizeof(response), 0, sizeof(response));
    robot_sle_execute_text(write_cb->value, write_cb->length, response, sizeof(response));
    (void)robot_sle_notify_text(conn_id, response);
    osal_printk("\r\nROBOT SLE cmd=%c len=%u response=%s", key, write_cb->length, response);
}
...
bool robot_sle_server_start(void)
{
    ...
    osal_printk("\r\nROBOT SLE start enter event_ready=%u\r\n", g_robot_sle_event_ready ? 1U : 0U);
    if (!g_robot_sle_event_ready) {
        osal_printk("\r\nROBOT SLE event init enter\r\n");
        if (osal_event_init(&g_robot_sle_event) != OSAL_SUCCESS) {
            osal_printk("\r\nROBOT SLE event init failed\r\n");
            return false;
        }
        g_robot_sle_event_ready = true;
        osal_printk("\r\nROBOT SLE event init ok\r\n");
    }

    osal_printk("\r\nROBOT SLE worker create enter\r\n");
    osal_kthread_lock();
    ...
    osal_kthread_unlock();
    osal_printk("\r\nROBOT SLE worker create done worker=%p\r\n", worker);
    ...
    osal_printk("\r\nROBOT SLE register callbacks enter\r\n");
    robot_sle_register_ssaps_callbacks();
    robot_sle_register_conn_callbacks();
    robot_sle_register_announce_callbacks();
    osal_printk("\r\nROBOT SLE wait stack ready %u ms\r\n", ROBOT_SLE_START_DELAY_MS);
    osal_msleep(ROBOT_SLE_START_DELAY_MS);
    osal_printk("\r\nROBOT SLE enable_sle enter\r\n");
    (void)enable_sle();
    osal_printk("\r\nROBOT SLE enable_sle returned\r\n");
