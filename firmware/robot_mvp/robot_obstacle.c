...
bool robot_obstacle_read(robot_obstacle_data_t *output)
{
    ...
    distance_mm = (pulse_us * ROBOT_OBSTACLE_SOUND_NUMERATOR) / ROBOT_OBSTACLE_SOUND_DENOMINATOR;
    ...
    output->blocked = (output->distance_mm > 0U) && (output->distance_mm < ROBOT_OBSTACLE_THRESHOLD_MM);
    return true;
#else
    return false;
#endif
