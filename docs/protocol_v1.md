...
- 当前板端已启用 HC-SR04 GPIO 驱动：TRIG=`GPIO_00`，ECHO=`GPIO_01`，阈值 `250 mm`。
...
- `AT+ROBOTAVOIDTEST` 不读取真实 HC-SR04，而是在板端模拟一次 `120 mm` 障碍，再切换到 `450 mm` 安全距离。
- 该命令会真实驱动电机，默认持续约 `1800 ms` 后自动停车；`S` / `AT+ROBOTS` 可随时打断。
- 用途：在超声波模块未到货或未接线时，验证自动避障状态机和电机动作链路。
...
SLE/星闪第一版控制 profile 见 `D:\aaa嵌入式比赛\protocol_docs\sle_control_profile.md`。SLE 初期建议先使用 ASCII 兼容模式：central 写入 `F/B/L/R/S/I/O/E/D/A/X/T`，板端 notify 本文定义的 `+ROBOT:*` 响应；待连接和安全动作稳定后，再切换到下方二进制控制包做 1000 包 RTT 基线测试。
