...
+ROBOT:OBS,<enabled>,<valid>,<blocked>,<distance_mm>,<threshold_mm>\r\n
+ROBOT:AVOID,<active>,<phase>,<status>,<enabled>,<valid>,<blocked>,<distance_mm>,<threshold_mm>\r\n
+ROBOT:AVOIDTEST,<active>,<phase>,<status>,<enabled>,<valid>,<blocked>,<distance_mm>,<threshold_mm>,<duration_ms>\r\n
```
...
- `RobotSle` 工作线程执行 `F/B/L/R/S/I/O/E/D/A/X/T` 后 notify 对应 `+ROBOT:*` 响应；
- `conn_id=0` 已按合法连接处理，不能再当作未连接。

SLE write callback 不应直接操作 `dut_motor.c`，而应调用：
...
- `robot_mvp_control_start_avoid_test()`：处理 `X`
...
```text
D:\aaa嵌入式比赛\upper_client\robot_profile.py
```

未来实现 SLE central 时，ASCII 写入命令应复用 `encode_sle_ascii_command()`，不要重新手写一套 `F/B/L/R/S/I/O/E/D/A/X/T` 映射。

当前 SLE bridge 调试包：

```text
D:\aaa嵌入式比赛\artifacts\ws63-liteos-app_mvp_sle_bridge_sdk_load_only_boot_verified.fwpkg
```
...
5. 写入 `X`：模拟避障动作与已验证 AT 版本一致；
6. 统计 1000 次 `T` 或 `F,S` 的 RTT、P95、P99、最大值、丢包率；
7. 开启树莓派视频后重复第 6 步，形成 SLE 控制与视频拥塞对比材料。
