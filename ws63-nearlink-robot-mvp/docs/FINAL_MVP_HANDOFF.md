...
参考 `D:\aaa嵌入式比赛\HiSparkEP_2025-02-10_pinout.md`：
...
| `D:\aaa嵌入式比赛\upper_client` | Python 上位机串口/TCP 客户端 | 是 |
| `D:\aaa嵌入式比赛\tests` | Python 自动化测试 | 是 |
| `D:\aaa嵌入式比赛\protocol_docs` | 协议文档 | 是 |
| `D:\aaa嵌入式比赛\docs` | 工程交接、硬件和流程文档 | 是 |
| `D:\aaa嵌入式比赛\artifacts` | 最终保留产物 | 是 |
| `D:\aaa嵌入式比赛\ws63_liteos` | 早期参考代码镜像 | 否 |
| `D:\aaa嵌入式比赛\deliverables` | 历史交付包 | 否，保留追溯 |
| `D:\aaa嵌入式比赛\github_repo` | GitHub/Gitee 参考仓库整理 | 否，保留参考 |

后续修改规则：

- 改板端行为，只改 `D:\w\src\application\samples\peripheral\robot_mvp`，必要时改 `main.c` 启动入口。
- 改上位机串口工具，只改 `D:\aaa嵌入式比赛\upper_client`、`scripts`、`tests`。
- 改协议，先改 `D:\aaa嵌入式比赛\protocol_docs\protocol_v1.md`。
...
```text
D:\aaa嵌入式比赛\artifacts\ws63-liteos-app_mvp_real_hcsr04_load_only.fwpkg
```
