...
- `RobotSle` 工作线程执行 `F/B/L/R/S/I/O/E/D/A/T` 后 notify 对应 `+ROBOT:*` 响应；
...
未来实现 SLE central 时，ASCII 写入命令应复用 `encode_sle_ascii_command()`，不要重新手写一套 `F/B/L/R/S/I/O/E/D/A/T` 映射。
...
6. 在安全场地写入 `A`，再用 `S` 急停：验证真实超声波避障链路和停车优先级；
7. 统计 1000 次 `T` 或 `F,S` 的 RTT、P95、P99、最大值、丢包率；
8. 开启树莓派视频后重复第 7 步，形成 SLE 控制与视频拥塞对比材料。
